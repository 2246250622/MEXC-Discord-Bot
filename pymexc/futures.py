...
class HTTP(_FuturesHTTP):
    ...
    def open_positions(self, symbol: Optional[str] = None) -> dict:
        """
        ### Get the user's open position information
        #### Required permissions: Trade reading permissions

        Rate limit: 20 times / 2 seconds

        https://mxcdevelop.github.io/apidocs/contract_v1_en/#get-the-user-39-s-open-position-information
        ...
        :rtype: dict
        """
        message = self.call("GET", "api/v1/private/position/open_positions", 
                            params = dict(
                                    symbol = symbol
                            ))
        result = {}
        for item in message['data']:
            if item['positionType'] == 1:
                ...
                position_type = "空"
            symbol = "幣種："+item['symbol']
            hold_avg_price_fully_scale = position_type+item['holdAvgPriceFullyScale']
            result[symbol] = hold_avg_price_fully_scale
